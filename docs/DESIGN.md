...
当前版本的控制台交互采用轻量现代 CLI 风格，而不是全屏 TUI。

交互设计原则：

1. 使用中文引导文案面向普通用户
2. 保留少量必要英文技术词，如 `BOT`、`Game`、`DLL`
3. 用统一的阶段标题展示流程进度，并在进入新阶段时重绘单独屏幕
4. 用编号菜单处理目录选择、覆盖模式和冲突决策
5. 在顶部保留之前阶段的关键信息，帮助用户理解当前上下文
6. 在流程结束时输出稳定的结果摘要，并在短暂停留后自动退出
...
3. `! ` 表示可继续的提醒或警告
4. `X ` 表示当前流程失败
5. 颜色用于辅助区分语义：阶段使用强调色，成功使用绿色，警告使用黄色，失败使用红色

#### Copy Style

文案风格约束如下：

1. 整体语气面向普通用户，而不是面向开发者
2. 优先使用自然中文表达，而不是直接暴露内部枚举名
3. 保留少量必要技术词，如 `BOT`、`Game`、`DLL`
4. 所有可选操作统一使用编号菜单
5. 所有流程结束都应收口到摘要或失败说明
...
1. 最终摘要页不能一闪而过
2. 最终界面停留约 3 秒后自动关闭 CLI
3. 成功摘要页与失败页都应遵循相同的单屏展示原则
4. 成功页需要有更明确的成功标识，例如 `注入完成`

### Logging

日志文件默认记录 `INFO`、`WARN`、`ERROR`，并额外记录更细的 `DEBUG` 信息用于排障。

`DEBUG` 内容应重点覆盖：

1. 输入来源和关键路径解析结果
2. 目标目录搜索过程中的候选项与命中数量
3. 压缩包扫描过程中的条目筛选和映射路径
4. 每个文件的写入决策：新增、覆盖、跳过
5. 后处理中的卡密写入和 DLL 重命名细节
...
成功结束时应输出稳定的结果摘要，方便普通用户快速判断结果，也方便截图和远程排障。
...
X 注入失败
  ...
  失败原因：
  压缩包中未找到有效目录：league of legends/

  日志文件：
  D:\project\bat_bot\tf_bot_injector_20260407_201533.log
```
...
1. 将所有阶段输出统一成一致的现代命令行风格
2. 将 BOT 卡密、覆盖策略、文件冲突、目录选择等交互统一为中文编号菜单
